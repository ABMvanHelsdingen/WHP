...
Type objective_function<Type>::operator() () {
  ...
  PARAMETER(log_k2);
  PARAMETER(logit_p);
  PARAMETER(logit_mean1);

  Type beta = exp(log_beta);
  Type alpha = exp(logit_abratio) / (Type(1.) + exp(logit_abratio)) * (beta/marks_mean); // enforcing 0<=alpha<=beta
  Type k1 = exp(log_k1); // k1 > 0
  Type k2 = exp(log_k2); // k2 > 0
  Type p = exp(logit_p) / (Type(1.) + exp(logit_p));
  p = (Type(0.5)*p) + Type(0.5);
  Type mean1 = exp(logit_mean1) / (Type(1.) + exp(logit_mean1));
  
  Type lambda1 = mean1 / (exp(lgamma(Type(1.) + (Type(1.)/k1))) * p);
  Type lambda2 = (Type(1.) - mean1) / (exp(lgamma(Type(1.) + (Type(1.)/k2))) * (Type(1.) - p));
  ...
  for(int i = 0; i < N; ++i){
    Type pdf = p*dweibull(diff_compensators[i], k1, lambda1) +
      (Type(1.) - p)*dweibull(diff_compensators[i], k2, lambda2);
    nll = nll - log(pdf);
  }
  //for(int i = 0; i < N; ++i){
